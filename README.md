...
#### Collision Group Registration (server)
...
treating `name` as a hitbox group. Roblox has no API to unregister a
`PhysicsService` group, so the group itself still exists — only Central's
bookkeeping and the collidability rule are undone.
