...
quality — a `partial_rebuild` restructuring only the subtrees a change
actually touched in the routine case, `full_rebuild` a floor for drift that
can't be spot-fixed that way (see
[Refitting instead of rebuilding](#refitting-instead-of-rebuilding-the-rebuild-path-rework),
not yet ported from the vendored copy this was developed in). Bounds are
cheap and change every capture, so each sample stores nothing but a pair of
`{vector}` arrays indexed by that tree's node indices. A capture writes each
part's exact AABB into its leaf slot and runs one bottom-up pass to fill the
internal nodes: linear, no restructuring, no allocation. Queries point the
tree's bounds arrays at the sample being queried, so the broad phase is
bolt's own traversal, unmodified.
...
One thing the old section got right: `"refit"`'s shared tree used to pay a
real, rare periodic-rebuild spike — observed up to ~2-5× a typical capture
(500+ µs against a ~110-260 µs steady state at 800 hitboxes) in this session's
own measurements. The table above is the minimum across many batches
specifically to filter that stall out and report steady-state cost; it was
real, and in the tail, not the typical frame — see
[Refitting instead of rebuilding](#refitting-instead-of-rebuilding-the-rebuild-path-rework)
for how that tail was cut down.
...
### Refitting instead of rebuilding: the rebuild-path rework

The rebuild spike above came from `"refit"` invalidating every internal node
in every sample on any topology change — `full_rebuild` every
`TREE_REBUILD_CHECK_INTERVAL` (10s), and every `AddPart`/`RemovePart` (the
frequent case: every character part spawning or despawning). Neither
genuinely needs to touch every node: a node's bounds in every sample are
already correct if its subtree is unchanged, and bolt's own `partial_rebuild`
only ever dissolves subtrees it flagged `enlarged`. The rework:

- an explicit dirty-set diff (`dirtyOrder`) against the topology snapshot from
  the last time every sample was fully consistent, so a stale sample's refit
  covers only the nodes actually touched by a change, not every internal node;
- `partial_rebuild` as the routine path, with a fixed `should_rebuild`
  bias (it compared stale internal envelope bounds against fresh leaf ones,
  which suppressed rebuilds) and a rebuild trigger that measures the sample
  just captured instead of the envelope; `full_rebuild` stays as the floor for
  drift `partial_rebuild`'s `enlarged` flag can't catch (a box that only ever
  shrinks);
- a drain budgeted in units of work (node refits) rather than paced against a
  wall-clock deadline, so its cost stays roughly constant regardless of how
  big the triggering change was, and drained newest-first so no budget is
  spent on a sample about to be overwritten anyway;
- the rebuild check itself staged across captures — a cheap ratio sample,
  then (only if that trips) the envelope sweep, then the rebuild — so a check
  that doesn't end up rebuilding never pays for the sweep at all, and the
  sweep and the rebuild never land on the same frame as each other.

**This isn't ported to `central/lib` yet** — it exists only in the vendored
copy under a consuming game's `Packages/`, uncommitted, so there's no commit
hash to cite. Everything below was measured against that code, `FRAME_CAP =
20`, otherwise shipped defaults, in Roblox Studio via `execute_luau`.

**Correctness first**, since this is fundamentally an invalidation
optimisation and the failure mode is a stale bound silently turning a hit
into a miss. Differentially tested against `"trees"` — 600 randomized
raycasts (grazing sweeps plus a ±0.02-stud tangency band, smaller trial counts
than the 300+500 above but the same method) at four points: steady state,
immediately after an `AddPart`, immediately after a `partial_rebuild`, and
immediately after a `full_rebuild`, each querying a sample still dirty
mid-drain. **Zero hit/miss disagreements, zero distance mismatches** at every
point. Separately, an internal invariant — refitting a dirty sample over its
full node set must equal refitting it over just `dirtyOrder` — held across
~10,000 node comparisons spanning `AddPart`, `RemovePart`, a second change
landing mid-drain, `partial_rebuild`, `full_rebuild`, and `AddPart`/
`RemovePart` landing mid-sweep. The self-heal property the drain's budget
depends on (every sample refit at least once every `FRAME_CAP` captures, with
no drain at all beyond its floor of one sample) was confirmed directly too.

**The costs, isolated** (800 hitboxes, minimum across batches):

| | before | after |
|---|---|---|
| rebuild-check cost when nothing trips | full envelope sweep every check | ~26 µs (one ratio sample; the sweep no longer runs at all) |
| `AddPart` fallout (one full `FRAME_CAP`-capture window) | ~1050-1070 µs | ~366 µs |

The idle-check number lands almost exactly on the back-of-envelope estimate
(~25 µs) from the profiling this rework started from. The `AddPart` number
dropping by ~3× rather than the ~80× the `dirtyOrder`-size ratio alone would
suggest is expected: most of a `FRAME_CAP`-capture window's cost is samples
reaching their own natural capture turn regardless of `AddPart`, which
`dirtyOrder` doesn't touch — the drain, and the `AddPart`-following samples it
would otherwise have paced, are what shrank.

**Does the tail actually shrink live?** Bound at `Settings.StepFrequency` /
`Settings.HitboxStepPriority` exactly as `CentralServer.Start()` does, 800
hitboxes, 152 samples over 5 real seconds with a rebuild forced mid-window:
min 315 µs, median 352 µs, **max 671 µs** — worst case only ~2.1× the
minimum. The old backend's live worst case was up to 10× its isolated
steady-state minimum (the section above); staging the rebuild check so its
costs never land on the same frame is what closes most of that gap, since the
single-frame spike a live 10× tail came from no longer exists as a single
frame's cost.
...
| `PARTIAL_REBUILD_RATIO_THRESHOLD` | `1.5` | `refit` backend only. How far a rebuild check's SAH-cost ratio is allowed to drift above the ratio recorded at the last rebuild before triggering a `partial_rebuild`. |
| `FULL_REBUILD_RATIO_THRESHOLD` | `3.0` | `refit` backend only. Same drift check as above, but for a `full_rebuild` — the floor for drift `partial_rebuild`'s `enlarged` flag can't catch (a box that only ever shrinks). |
